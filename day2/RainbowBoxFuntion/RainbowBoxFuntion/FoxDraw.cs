...
namespace GreenFox
{
    public class FoxDraw
    {
        ...
        public void DrawSquare(double input)
        {
            FillColor(Colors.Transparent);
            Random random = new Random();
            double size = Canvas.Height;
            double size2 = Canvas.Width;
            while (size != input)
            {
                size -= 25;
                size2 -= 25;
                double left = (Canvas.Width - size) / 2;
                double top = (Canvas.Height - size) / 2;
                Color randomColor = Color.FromRgb(
                    Convert.ToByte(random.Next(256)),
                    Convert.ToByte(random.Next(256)),
                    Convert.ToByte(random.Next(256)));
                FillColor(randomColor);
                DrawRectangle(left, top, size, size);
            }
